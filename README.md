...
当前项目没有内置音频文件时，会自动使用网页内置的 Web Audio beep 声作为备用提醒。手机浏览器通常要求用户先点击页面，所以老板第一次打开后台后，需要点一次“开启声音提醒”。

提醒设置默认收起在老板后台顶部的“提醒设置”里，不长期占用订单页面。手机端如果想弹出系统通知，需要从 Safari/Chrome 打开后台、允许通知，最好再添加到主屏幕使用。注意：网页提醒不能保证和微信原生消息一样 100% 后台常驻。如果手机锁屏、浏览器被系统关闭、网页没有打开，网页声音、标题闪烁和图标角标可能不会触发。
