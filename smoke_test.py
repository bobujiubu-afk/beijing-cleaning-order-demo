...
def main():
    ...
    assert admin.status_code == 200 and "老板接单本登录".encode("utf-8") in admin.data
    ...
    assert login.status_code == 200 and "今天的活".encode("utf-8") in login.data
    assert "已报价".encode("utf-8") not in login.data
    assert "已成交".encode("utf-8") not in login.data
    assert "已约好".encode("utf-8") in login.data
    ...
    assert "新增一单".encode("utf-8") in login.data
    assert "负责人".encode("utf-8") not in login.data
    assert "回访状态".encode("utf-8") not in login.data
    ...
    assert "预约二维码".encode("utf-8") not in client.get("/admin").data
    ...
    manual = client.post(
        "/orders/create",
        data={
            "customer_name": "手动录入客户",
            "phone": "13811112222",
            "address": "朝阳区测试小区",
            "service_type": "日常保洁",
            "area": "2小时",
            "appointment_date": date.today().strftime("%Y-%m-%d"),
            "appointment_period": "上午",
            "amount": "300",
            "status": "已约好",
            "source": "微信沟通",
            "remark": "",
    ...
    assert "手动录入客户".encode("utf-8") in manual.data
    assert "微信沟通".encode("utf-8") in manual.data
    ...
    assert "上门地址" in headers
    assert "面积/数量" in headers
    ...
    exported_statuses = [row[status_column - 1].value for row in workbook.active.iter_rows(min_row=2)]
    assert "已报价" not in exported_statuses
    assert "已成交" not in exported_statuses
